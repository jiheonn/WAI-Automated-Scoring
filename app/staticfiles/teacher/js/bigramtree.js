function resize() {
  width = vis.node().clientWidth, height = window.innerHeight - 50 - 0, heatmap.attr("transform", "translate(" + (width - 20.5) + ",.5)").select("rect.frame").attr("height", height - 1), svg.attr("width", width).attr("height", height), clip.attr("width", width - 30.5).attr("height", height), treeG.call(tree.width(width - 30).height(height - 20)), text.call(textViewer)
}
...
function change(sentences=null, args=null) {
  if (!location.search) return void hideOption();
  ...
  if (sentences!=null) {
    ...
    if (tokens && tokens.length) {
      ...
      }).reverse(+state.reverse).phraseLine(+state["phrase-line"]).prefix(t)), refreshText(tree.root()), hideOption()
    }
  } else {
    if (state = urlParams(location.search.substr(1)), state.source !== e && state.source) source.property("value", state.source);
...
function showOption() {
  d3.selectAll("#form-source").style("display", null), d3.selectAll("#form, #reverse-wrapper").style("display", null)
}

function hideOption() {
  d3.selectAll("#form-source").style("display", "none"), d3.selectAll("#form, #reverse-wrapper").style("display", "inline-block")
}
    ...
    function e(e) {
      function o(o) {
        e.each(function() {
          ...
          var s = e.select(".current").selectAll("div.row_b").data(d3.range(c, Math.min(l, n)), String);
          s.enter().append("div").attr("class", "row_b"), s.exit().remove(), s.order().call(t)
        })
      }
...
d3.select("#paste-go").on("click", function() {
  url(state = {source: ""}, !0),
  d3.select("#paste-save").property("checked") && d3.text("save").post(d3.select("#paste").property("value"),
  function(e, t) {
